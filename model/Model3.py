...
def inception_v4(nb_classes=1001, load_weights=True):
    ...
    init = Input((224, 224, 3))  # Channels last, as using Tensorflow backend with Tensorflow image dimension ordering
    ...
    # Average Pooling
    x = AveragePooling2D((5, 5))(x)  # Output: 1536
...
if __name__ == "__main__":
    ...
    trainGen = trainImageDataGen.flow_from_directory(os.path.join(root_path, "..", "data", "dataset", "model3", "train"),
        target_size=(224,224),
        batch_size=64,
        class_mode="categorical")

    valGen = validationImageDataGen.flow_from_directory(os.path.join(root_path, "..", "data", "dataset", "model3", "validation"),
        target_size=(224,224),
        batch_size=64,
        class_mode="categorical")
    model = inception_v4(nb_classes=5)
